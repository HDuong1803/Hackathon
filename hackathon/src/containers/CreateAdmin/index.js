...
import getSCEthereumVaccineUserAttend from "../../web3/VaccineUserAttend";
import { ToastContainer, toast } from "react-toastify";
import { SERVER, TRANSACTION_STATUS } from "../../constants/Config";
...
export default function CreateAdmin() {
  ...
  const [vaccineUserAttendContract, setVaccineUserAttendContract] =
    useState(undefined);
  ...
  const handleIsActive = (value) => setisActiveValue(value);
  const handleProfileHash = (text) => setprofileHashValue(text.target.value);

  const onConnectWallet = async () => {
    const { vaccineSPSC } = await getSCEthereumVaccineUserAttend();
    setVaccineUserAttendContract(vaccineSPSC);
  ...
  const updateUserAdminData = async () => {
    if (accounts) {
      const active = isActiveValue === "true";
      const transaction = await vaccineUserAttendContract.updateUserForAdmin(
        userAddressValue,
        nameValue,
        contactNoValue,
        roleValue,
          ...
          <div className="create-process">
            <div className="create-process_left">
              ...
              <div className="flex flex-col space-y-2 mb-4">
                ...
                >
                  ...
                  <Option value={"VACCINATION_STATION"}>
                    VACCINATION STATION
                  </Option>
            ...
            <div className="create-process_right">
              <div className="create-process_right_contain">
                ...
                <p className="create-process_right_contain--subtitle pb-2">
                  {t("adminForm.name")}{" "}
                  <strong>{nameValue ? nameValue : emtyValue}</strong>
                </p>
                <p className="create-process_right_contain--subtitle pb-2">
                  {t("adminForm.contactNo")}{" "}
                  <strong>{contactNoValue ? contactNoValue : emtyValue}</strong>
                </p>
                <p className="create-process_right_contain--subtitle pb-2">
                  {t("adminForm.role")}{" "}
                  <strong>{roleValue ? roleValue : emtyValue}</strong>
                </p>
                <p className="create-process_right_contain--subtitle pb-2">
                  {t("adminForm.profileHash")}{" "}
                  <strong>
