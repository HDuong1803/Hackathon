...
import { transactionDetail } from './transaction.provider'
...
const synchronizeVaccineSupplyChain = async (
  ...
  for (const completeBasicDetail of listCompleteBasicDetail) {
    try {
      ...
      const txnData = await transactionDetail(completeBasicDetail.transactionHash)
      ...
      await Process.create({
        ...
        from: txnData.data.result.from,
        to: txnData.data.result.to,
        transactionHash: txnData.data.result.transactionHash,
        status: Number(txnData.data.result.status) ? 'SUCCESS' : 'UNSUCCESS',
        blockHash: txnData.data.result.blockHash,
        blockNumber: txnData.data.result.blockNumber,
        transactionIndex: txnData.data.result.transactionIndex,
        contractAddress: txnData.data.result.logs[0].address,
  ...
  for (const completeWarehouser of listCompleteWarehouser) {
    try {
      ...
      const txnData = await transactionDetail(completeWarehouser.transactionHash)
      ...
      await Warehouser.create({
        ...
        nextAction: Constant.NEXT_ACTION.DISTRIBUTOR
      })
    } catch (error: any) {
      logger.error(
  ...
  for (const completeDistributor of listCompleteDistributor) {
    try {
      ...
      const txnData = await transactionDetail(completeDistributor.transactionHash)
      ...
      await Distributor.create({
        ...
        transactionIndex: txnData.data.result.transactionIndex,
        contractAddress: txnData.data.result.logs[0].address,
        nextAction: Constant.NEXT_ACTION.VACCINATION_STATION
      })
    } catch (error: any) {
      logger.error(
  ...
  for (const completeVaccinationStation of listCompleteVaccinationStation) {
    try {
      ...
      const txnData = await transactionDetail(completeVaccinationStation.transactionHash)
  ...
  for (const completeVaccinatePerson of listCompleteVaccinatePerson) {
    try {
      ...
      const txnData = await transactionDetail(completeVaccinatePerson.transactionHash)
